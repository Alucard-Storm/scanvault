...
class EncryptionService {
  ...
  static Future<String> generateKeyForFolder(String folderId) async {
    ...
    await _storage.write(key: 'folder_key_$folderId', value: keyString);
  ...
  static Future<void> decryptFile(String filePath, String folderId) async {
    try {
      ...
      final iv = enc.IV(Uint8List.fromList(allBytes.sublist(4, 20)));

      // Extract encrypted data (remaining bytes after header + IV)
      final encrypted = enc.Encrypted(Uint8List.fromList(allBytes.sublist(20)));
  ...
  static Future<void> encryptFiles(
  ...
  static Future<void> decryptFiles(
    List<String> filePaths,
    String folderId,
  ) async {
    for (final filePath in filePaths) {
